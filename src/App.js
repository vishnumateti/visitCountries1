...
class App extends Component {
  ...
  addVisitedCountries = id => {
    ...
    const visitCountries = countriesList.map(country => {
      if (country.id === id) {
        const updateList = {...country, isVisited: !country.isVisited}
        return updateList
      }
      return country
    })
  ...
  removeVisitedCountry = id => {
    const {countriesList} = this.state
    const filteredCountries = countriesList.map(eachCountry => {
      ...
      <div className="bg-container">
        ...
        <ul className="countries-container">
          {countriesList.map(eachCountry => (
            <CountriesList
              ...
              addVisitedCountries={this.addVisitedCountries}
            />
          ))}
        </ul>
        ...
        {updatedList.length > 0 ? (
          <ul className="visited-countries-container">
            {updatedList.map(visitedCountry => (
              <VisitedCountries
                key={visitedCountry.id}
                visitedCountryDetails={visitedCountry}
                removeVisitedCountry={this.removeVisitedCountry}
              />
            ))}
          </ul>
        ) : (
          <div className="no-visited-contries-container">
